...
//	static int SCREEN_HEIGHT = 900;
	...
	private void paintMenu(Canvas canvas) {
		...
		DrawTool.paintImage(canvas, mResourceStore.getMenu(), 0, SCREEN_HEIGHT
				/ 2 - mResourceStore.getMenu().getHeight() / 2);
			...
			for (j = 0; j < 4; j++) {
				if (mNextTile.mTile[i][j] != 0) {
					DrawTool.paintImage(
							...
							(int) (Court.BEGIN_DRAW_X
									+ getBlockDistance(Court.COURT_WIDTH) + getBlockDistance((float) (i + 0.5))),
	...
	private void paintSpeed(Canvas canvas) {
		...
		canvas.drawText("等级:", getBlockDistance(Court.COURT_WIDTH)
				+ getRightMarginToCourt(), getBlockDistance(9), mPaint);
		mPaint.setColor(Color.RED);
		canvas.drawText(String.valueOf(mSpeed),
				getBlockDistance(Court.COURT_WIDTH) + 2
						* getRightMarginToCourt(), getBlockDistance(11), mPaint);
	}

	private void paintScore(Canvas canvas) {
		mPaint.setColor(Color.BLUE);
		canvas.drawText("得分:", getBlockDistance(Court.COURT_WIDTH)
				+ getRightMarginToCourt(), getBlockDistance(13), mPaint);
		mPaint.setColor(Color.RED);
		canvas.drawText(String.valueOf(mScore),
				getBlockDistance(Court.COURT_WIDTH) + 2
						* getRightMarginToCourt(), getBlockDistance(15), mPaint);
	}

	private void paintDeLine(Canvas canvas) {
		mPaint.setColor(Color.BLUE);
		canvas.drawText("消去行数:", getBlockDistance(Court.COURT_WIDTH)
				+ getRightMarginToCourt(), getBlockDistance(17), mPaint);
		mPaint.setColor(Color.RED);
		canvas.drawText(String.valueOf(mDeLine),
				getBlockDistance(Court.COURT_WIDTH) + 2
						* getRightMarginToCourt(), getBlockDistance(19), mPaint);
	}
	...
	private void paintOver(Canvas canvas) {
		...
		canvas.drawText("Game Over", getBlockDistance(1),
				getBlockDistance(Court.COURT_HEIGHT / 2 - 2), paint);
	...
	private void restoreCourt(Properties pro) {
		...
		for (i = 0; i < Court.COURT_WIDTH; i++) {
			for (j = 0; j < Court.COURT_HEIGHT; j++) {
				matrix[i][j] = Integer.valueOf(pro.get("courtMatrix" + i + j)
						.toString());
			}
		}
	}
	...
	private void restoreTile(Properties pro, TileView tile) {
		...
		for (i = 0; i < 4; i++) {
			for (j = 0; j < 4; j++) {
				matrix[i][j] = Integer.valueOf(pro.get("tileMatrix" + i + j)
						.toString());
			}
		}
	...
	public void saveGame() {
		...
		try {
			FileOutputStream stream = mContext.openFileOutput(DATAFILE,
